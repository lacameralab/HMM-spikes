function [guessTR,guessE,converged,logliks,rec] = myhmmtrain(seqs,guessTR,guessE,varargin)
% [guessTR,guessE,converged,logliks,rec] = myhmmtrain(seqs,guessTR,guessE,varargin)
% This function is edited from MATLAB function hmmtrain by adding some output.
%   converged: if 1, the algorithm converges; if 0, not converged.
%   rec: record the diagonal of transition matrix during training.
% 
...
rec.TR_diag = zeros(numStates,maxiter*numSeqs);
rec.TR_diag(:,1) = diag(guessTR);
...
for iteration = 1:maxiter
    ...
    rec.TR_diag(:,iteration+1) = diag(guessTR);
