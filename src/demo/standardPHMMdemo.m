% This file is a demo for applying standard Poisson-HMM to spiking data.
% 
% NOTICE:
...
%   2. You can change the way of choosing the initial firing rate matrix (lambda_guess).
...
%   3. You can change the way of setting the initial transition matrix (Gamma_guess).
%      This demo set the initial self-transition probability of each state
%      to be higher than 0.8. Each entry of Gamma_guess should be value
...
Gamma_guess = rand(m); Gamma_guess = 0.8*eye(m)+(1-0.8)*Gamma_guess./sum(Gamma_guess,2); % initial value for transition matrix

%% ========================     Poisson-HMM     ===========================
[estlambda,estGamma,isconverge,delta1,logliks,rec,iter,crit] = phmm_EM(spkc(train_ind),m,lambda_guess,Gamma_guess,0);
...
title('Self transition probability during training (standard Poisson-HMM)');
hold off;
...
for k = 1:min(3,nTrial)
    ...
    title(sprintf('standard Poisson-HMM (N=%i), m=%i, dt=%i ms, trial %i',N,m,dt*1000,k),'fontsize',fntsz+2,'fontweight','bold');
...
sgtitle(sprintf('Standard Poisson-HMM, %s',replace(datafilename,'_','\_')));
